...
document.addEventListener('click', (e) => {
  // Skip DataTables responsive control clicks
  if (e.target.closest('.dtr-control') || e.target.closest('td.dtr-control') || e.target.closest('.dtr-modal')) return;
...
async function initRings() {
    const host = document.getElementById('resourceRings');
...
async function updateRings() {
    try {
      ...
      const values = [
    data.server.cpu.percent,
    data.server.ram.percent,
    data.server.disk.percent
];

document
    .querySelectorAll('#resourceRings .ring-item')
    .forEach((item, index) => {

        const percent = Number(values[index]) || 0;

        const circle = item.querySelector('.ring-fill');
        const text = item.querySelector('.ring-center-text');

        // warna berdasarkan persen
        circle.classList.remove('green', 'amber', 'red');
        circle.classList.add(getRingColor(percent));
...
function whpConfirm(title, msg, onConfirm, btnText) {
  ...
  overlay.innerHTML = `
    <div class="confirm-box">
      ...
      <div class="confirm-actions">
        ...
        <button class="btn-danger-soft" data-c="ok">${btnText || 'Delete'}</button>
      </div>
    </div>`;
...
// ---- Loading Overlay ----
function whpLoading(title, msg) {
  const overlay = document.createElement('div');
  overlay.className = 'confirm-overlay';
  overlay.id = 'whp-loading-overlay';
  overlay.innerHTML = `
    <div class="confirm-box">
      <div class="confirm-icon" style="background:var(--primary-light)">
        <i class="fa-solid fa-spinner fa-spin" style="color:var(--primary);font-size:1.5rem"></i>
      </div>
      <h3 class="confirm-title">${title || 'Processing...'}</h3>
      <p class="confirm-msg">${msg || 'Please wait...'}</p>
      <div style="width:100%;height:4px;background:var(--bg-secondary);border-radius:4px;overflow:hidden;margin-top:8px">
        <div style="width:30%;height:100%;background:var(--primary);border-radius:4px;animation:loadingBar 1.5s ease-in-out infinite"></div>
      </div>
    </div>`;
  document.body.appendChild(overlay);

  const style = document.createElement('style');
  style.textContent = '@keyframes loadingBar{0%{width:0%;margin-left:0}50%{width:60%;margin-left:20%}100%{width:0%;margin-left:100%}}';
  document.head.appendChild(style);

  return {
    update: function(newTitle, newMsg) {
